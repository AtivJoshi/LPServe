...
4. Consult `docs/lpserve_scheduler_architecture.md` for audited framework constraints and inspect current code whenever a framework claim affects the conclusion.
...
Before editing, record the branch, commit, and working-tree state, preserve unrelated user changes, and compare scheduler-relevant code with the audit baseline. For Phase D, start with Sections 7, 9–11, 14–18 of `docs/lp_scheduler_design.md` and the corresponding ILP/relaxation/extraction sections of `docs/math/main-llm-serving.tex`.
